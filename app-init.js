...
function showWhatsNew() {
  ...
  overlay.style.cssText = 'position:fixed; inset:0; background:rgba(0,0,0,0.75); backdrop-filter:blur(6px); -webkit-backdrop-filter:blur(6px); z-index:99999; display:flex; align-items:center; justify-content:center; padding:20px;';
  overlay.innerHTML = `<div style='background:var(--bg-base); border:1px solid var(--border-color); border-radius:18px; max-width:420px; width:100%; max-height:85vh; display:flex; flex-direction:column; overflow:hidden; box-shadow:0 20px 60px rgba(0,0,0,0.55);'><div style='padding:20px 20px 12px; text-align:center;'><div style='font-size:30px;'>🎉</div><div style='font-weight:800; color:var(--text-1); font-size:1.15rem; margin-top:6px;'>Có Gì Mới?</div><div style='color:var(--text-2); font-size:0.82rem; margin-top:4px;'>Cập nhật các tính năng mới nhất của ứng dụng</div></div><div style='padding:0 20px; overflow-y:auto; flex:1;'>${itemsHTML}</div><div style='padding:14px 20px 18px;'><label style='display:flex; align-items:center; gap:8px; font-size:0.8rem; color:var(--text-2); margin-bottom:12px; cursor:pointer;'><input type='checkbox' id='whatsNewDontShow' style='width:16px; height:16px;'> Không hiển thị lại lần sau</label><button id='whatsNewCloseBtn' class='btn-save' style='width:100%;'>Đã hiểu</button></div></div>`;
